		...
		public static ID3v2FrameEntry GetFrameEntry(string id, ID3Version version)
		{
			switch (version)
			{
				...
				case ID3Version.ID3v2_2:
					return Frames.Where((x) => !String.IsNullOrEmpty(x.ID3v2ID) && x.ID3v2ID.Equals(id)).First();

				case ID3Version.ID3v2_3:
					return Frames.Where((x) => !String.IsNullOrEmpty(x.ID3v3ID) && x.ID3v3ID.Equals(id)).First();

				case ID3Version.ID3v2_4:
					return Frames.Where((x) => !String.IsNullOrEmpty(x.ID3v4ID) && x.ID3v4ID.Equals(id)).First();
